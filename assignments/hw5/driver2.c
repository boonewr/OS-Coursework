...
void *lupeThread( void *arg ) {
  const char *name = "Lupe";
...
// A player with skill level 20
void *alidaThread( void *arg ) {
  const char *name = "Alida";

  // Try to play after 2 seconds.
  sleep( 2 );
  
  printf( "%s wants to play\n", name );
  if ( findMatch( name, 20 ) ) {
    printf( "%s is playing\n", name );
...
// A player with skill level 12
void *nickThread( void *arg ) {
...
// A player with skill level 8
void *micahThread( void *arg ) {
  const char *name = "Micah";
...
int main( int argc, char *argv[] ) {
  ...
  int i = 0;
  if ( pthread_create( thread + i++, NULL, lupeThread, NULL ) != 0 ||
       pthread_create( thread + i++, NULL, alidaThread, NULL ) != 0 ||
       pthread_create( thread + i++, NULL, nickThread, NULL ) != 0 ||
       pthread_create( thread + i++, NULL, micahThread, NULL ) != 0 )
